...
import { Radio, CheckCircle2, XCircle, Search, X, History, Users as UsersIcon } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setTodayLogs((prev) => [...prev, payload.new]);
        }
      )
      .subscribe();
                  ...
                  <tr key={m.id} className="hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => doPunch(m)}
                        disabled={st.count >= 2}
                        className="text-[10px] uppercase tracking-wider text-brand hover:underline disabled:text-muted-foreground disabled:no-underline"
                      >
...
function HistoryModal({ member, onClose }: { member: Member; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur grid place-items-center p-6" onClick={onClose}>
      <div className="w-full max-w-2xl bg-card border border-border rounded-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-border flex items-center gap-4">
          <img src={member.photo} alt="" className="size-12 rounded-xl object-cover" />
          <div className="flex-1">
            <h3 className="font-heading text-lg flex items-center gap-2">
              <History className="size-4 text-brand" /> {member.name}
            </h3>
            <p className="text-xs text-muted-foreground">{member.rollNo} · {member.rfid}</p>
          </div>
...
function CompareModal({ members, onClose, onClear }: { members: Member[]; onClose: () => void; onClear: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur grid place-items-center p-6" onClick={onClose}>
      <div className="w-full max-w-4xl bg-card border border-border rounded-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-border flex items-center gap-3">
          <UsersIcon className="size-5 text-brand" />
          <h3 className="font-heading text-lg flex-1">Compare Attendance ({members.length})</h3>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-5 space-y-4">
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className="p-4 bg-secondary/40 rounded-xl">
                ...
                <div className="flex gap-1">
                  {dayList.map(([day, count]) => (
                    <div
                      ...
                      className={`flex-1 h-8 rounded ${count === 0 ? "bg-secondary" : count === 1 ? "bg-brand/40" : "bg-brand"}`}
                    />
                  ))}
                </div>
              </div>
