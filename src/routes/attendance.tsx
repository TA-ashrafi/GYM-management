...
function Attendance() {
  ...
  const doPunch = async (m: Member) => {
    ...
    if (st.count >= 2) {
      toast.error(
        `${m.name} — Already punched IN & OUT today (maximum 2 punches allowed)`,
      );
      return;
    }
    ...
    if (!error) {
      ...
      toast.success(
        type === "IN" ? `✓ ${m.name} — Punch IN` : `👋 ${m.name} — Punch OUT`,
      );
        ...
        async (payload) => {
          ...
          if (row.branch_id === branchId && !row.claimed) {
            ...
            if (matchedMember) {
              ...
              toast.error(
                `Physical card scanned but not assigned to any member: "${row.uid}"`,
              );
            }
          }
        },
  ...
  const punch = (e: React.FormEvent) => {
    ...
    const m = members.find(
      (x) => x.rfid?.toLowerCase() === c || x.rollNo?.toLowerCase() === c,
    );
  ...
  return (
    <div className="p-4 sm:p-8 max-w-[1500px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 bg-gradient-to-br from-brand/15 to-card border border-brand/30 rounded-2xl p-4 sm:p-6">
          ...
          <p className="text-[11px] text-muted-foreground mt-3">
            1st scan = <span className="text-brand font-bold">IN</span> · 2nd
            scan = <span className="text-warn font-bold">OUT</span> · 3rd scan
            blocked
          </p>
        </div>
            ...
            <div className="flex items-center gap-4">
              {last.photo && (
                <img
                  src={last.photo}
                  alt=""
                  className="size-14 rounded-xl object-cover ring-2 ring-brand"
                />
              )}
              <div>
                <p className="text-lg font-heading">{last.name}</p>
                <p
                  className={`text-xs font-bold ${last.type === "IN" ? "text-brand" : "text-warn"}`}
                >
                  {last.type === "IN" ? "✓ Punch IN" : "👋 Punch OUT"} ·{" "}
                  {last.time}
                </p>
              </div>
            </div>
        ...
        <div className="p-4 border-b border-border flex flex-col gap-4 sm:flex-row sm:items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className="flex gap-1 items-center border-l border-border pl-3 ml-1">
              <span className="text-[10px] uppercase tracking-widest text-muted-foreground mr-1 hidden sm:inline">
                Range
              </span>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {filtered.map(({ m, st }) => {
                const rangeVisits = m.attendance.filter(
                  (a) => daysSince(a) < range,
                ).length;
                return (
                  <tr key={m.id} className="hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="flex items-center gap-3 text-left hover:text-brand"
                      >
                        {m.photo ? (
                          <img
                            src={m.photo}
                    ...
                    <td className="px-4 py-3">
                      {st.in ? (
                        <span className="inline-flex items-center gap-1.5 text-brand text-xs font-bold">
                          ...
                          {new Date(st.in).toLocaleTimeString("en-IN", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </span>
                    ...
                    <td className="px-4 py-3">
                      {st.out ? (
                        <span className="inline-flex items-center gap-1.5 text-warn text-xs font-bold">
                          👋{" "}
                          {new Date(st.out).toLocaleTimeString("en-IN", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </span>
              ...
              <div key={m.id} className="p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between text-xs px-2 py-1.5 bg-secondary/30 rounded-xl">
                  ...
                  <div className="flex flex-col gap-0.5 text-right">
                    <span className="text-[9px] uppercase tracking-wider text-muted-foreground">
                      Visits ({range === 1 ? "1d" : `${range}d`})
                    </span>
                    <span className="text-foreground font-semibold">
                      {rangeVisits} visit{rangeVisits !== 1 ? "s" : ""}
                    </span>
                  </div>
                </div>
      ...
      {/* Modals */}
      {historyOf && (
        <HistoryModal member={historyOf} onClose={() => setHistoryOf(null)} />
      )}
      {compareOpen && (
        <CompareModal
...
function HistoryModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-background/85 backdrop-blur-md grid place-items-center p-4 sm:p-6"
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="p-4 sm:p-5 border-b border-border flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">
              {member.rollNo} · {member.rfid}
            </p>
          </div>
          <button
            onClick={onClose}
            className="size-9 rounded-lg bg-secondary grid place-items-center hover:bg-danger/10 hover:text-danger shrink-0"
          >
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="max-h-[50vh] overflow-y-auto p-4 sm:p-5 space-y-3">
          ...
          {Object.entries(byDay).map(([day, times]) => (
            <div
              key={day}
              className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start pb-2 border-b border-border/20 last:border-0"
            >
...
function CompareModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-4xl bg-card border border-border rounded-3xl overflow-hidden shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-4 sm:p-5 border-b border-border flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-heading text-lg text-foreground">
              Compare Attendance ({members.length})
            </h3>
          </div>
          <div className="flex items-center gap-3 ml-auto">
            ...
            <button
              onClick={onClose}
              className="size-9 rounded-lg bg-secondary grid place-items-center hover:bg-danger/10 hover:text-danger"
            >
              <X className="size-4" />
            </button>
          </div>
        ...
        <div className="max-h-[50vh] overflow-y-auto p-4 sm:p-5 space-y-4">
          {members.map((m) => {
            const visits = m.attendance.filter(
              (a) => new Date(a).getTime() >= cutoff,
            );
            ...
            const rate = Math.round(
              (Object.values(dayBuckets).filter((v) => v > 0).length / days) *
                100,
            );
              ...
              <div key={m.id} className="p-4 bg-secondary/40 rounded-2xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xl font-heading text-brand">
                      {visits.length}
                    </p>
