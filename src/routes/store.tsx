...
function StorePage() {
  ...
  const totalProfit = sales.reduce((s, x) => {
    return (
      s +
      x.items.reduce((ii: number, it: any) => {
    ...
    <div className="p-8 max-w-[1600px]">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {filtered.map((p) => (
                <button
                  key={p.id}
                  onClick={() => addToCart(p)}
                  className="text-left p-4 bg-card border border-border rounded-xl hover:border-brand/40 hover:bg-card/80 transition"
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span
                      className={`text-[10px] px-2 py-1 rounded ${p.stock === 0 ? "bg-danger/15 text-danger" : p.stock <= (p.lowStockAt || 3) ? "bg-warn/15 text-warn" : "bg-brand/10 text-brand"}`}
                    >
                      {p.stock} left
                    </span>
                  </div>
                  <p className="text-sm font-semibold leading-tight mb-2 line-clamp-2 min-h-[2.5rem]">
                    {p.name}
                  </p>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 sticky top-24 h-fit">
              ...
              {cart.map((i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="size-6 rounded bg-secondary hover:bg-danger/10 hover:text-danger grid place-items-center"
                  >
                  ...
                  <button
                    onClick={() => updateQty(i.productId, +1)}
                    className="size-6 rounded bg-secondary hover:bg-brand/10 hover:text-brand grid place-items-center"
                  >
            ...
            <div className="border-t border-border pt-3 flex items-center justify-between mb-3">
              ...
              <span className="text-2xl font-heading text-brand">
                {money(total)}
              </span>
            </div>
            <button
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[10px] uppercase tracking-widest px-2 py-1 bg-secondary rounded">
                        {p.category}
                      </span>
                    </td>
                    ...
                    <td
                      className={`px-4 py-3 text-right ${margin > 0 ? "text-brand" : "text-danger"}`}
                    >
                      {money(margin)}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold ${low ? "bg-danger/10 text-danger" : "bg-brand/10 text-brand"}`}
                      >
                        {p.stock}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="size-8 grid place-items-center rounded bg-secondary hover:bg-brand/10 hover:text-brand inline-flex"
                      >
      ...
      {tab === "sales" && (
        <div className="space-y-2">
          ...
          {sales.map((s) => {
            const m = s.member_id
              ? members.find((x: any) => x.id === s.member_id)
              : null;
            return (
              <div
                key={s.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold">
                    {m?.name ?? s.customer ?? "Walk-in"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {new Date(s.created_at).toLocaleString()} · {s.items.length}{" "}
                    items · {s.payment_mode}
                  </p>
                </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-heading text-lg flex items-center gap-2">
                <Package className="size-4" /> {editing.id ? "Edit" : "New"}{" "}
                Product
              </h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Low Stock At">
                  <input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, lowStockAt: +e.target.value })
                    }
                  />
                </Field>
              </div>
...
function Stat({
  ...
  return (
    <div className="p-4 bg-card border border-border rounded-xl">
      ...
      <p
        className={`text-2xl font-heading mt-1 ${tone === "brand" ? "text-brand" : tone === "warn" ? "text-warn" : "text-foreground"}`}
      >
        {value}
      </p>
    </div>
