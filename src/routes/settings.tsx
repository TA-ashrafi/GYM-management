...
const CURRENCIES: Settings["currency"][] = [
  "INR",
  "USD",
  "AED",
  "PKR",
...
function SettingsPage() {
  ...
  const [workoutTemplates, setWorkoutTemplates] =
    useState<any[]>(SEED_TEMPLATES);
  ...
  function updateShift(i: number, patch: Partial<Shift>) {
    setForm((f) => ({
      ...f,
      shifts: f.shifts.map((s, idx) => (idx === i ? { ...s, ...patch } : s)),
    }));
  }
  ...
  function addShift() {
    setForm((f) => ({
      ...f,
      shifts: [...f.shifts, { start: "06:00", end: "10:00" }],
    }));
  }
  ...
  const addTemplate = () => {
    setWorkoutTemplates((prev) => [
      ...prev,
      {
        ...
        desc: "Custom 6-day split details",
        days: ["", "", "", "", "", ""],
      },
    ]);
  };
  ...
  async function save() {
    ...
    if (branchId) {
      ...
      localStorage.setItem(
        `fs_webhook_${branchId}`,
        form.whatsappWebhookUrl || "",
      );
      ...
      if (error) {
        if (error.message.includes("schema cache")) {
          ...
          toast.warning(
            "Settings saved locally! To enable cross-device sync, please run the Supabase query in SUPABASE_SQL.md.",
          );
          return;
        }
        toast.error("Error: " + error.message);
    ...
    <div className="p-4 sm:p-8 max-w-5xl w-full">
      <PageHeader
        ...
        actions={
          <button
            onClick={save}
            className="px-5 py-2.5 bg-brand text-brand-foreground font-semibold rounded-xl text-sm inline-flex items-center gap-2 w-full sm:w-auto justify-center cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-card border border-border rounded-2xl p-4 sm:p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {LANGS.map((l) => (
                <button
                  ...
                  className={
                    "px-4 py-2 rounded-lg text-sm border flex-1 text-center cursor-pointer " +
                    (form.language === l.v
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {CURRENCIES.map((c) => (
                <button
                  ...
                  className={
                    "px-3 py-2 rounded-lg text-sm border text-center cursor-pointer " +
                    (form.currency === c
                      ? "bg-brand text-brand-foreground border-brand font-medium"
                      : "bg-secondary border-border text-muted-foreground")
                  }
          ...
          <Field label="Session Duration (minutes)">
            <input
              ...
              onChange={(e) =>
                set("slotDurationMin", Math.max(15, +e.target.value))
              }
              className={input}
            />
          </Field>
          <Field label="Default Slot Capacity">
            <input
              ...
              onChange={(e) =>
                set("slotCapacity", Math.max(1, +e.target.value))
              }
              className={input}
            />
          </Field>
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden lg:col-span-2">
          ...
          >
            <h2 className="font-heading text-lg flex items-center gap-2 m-0">
              <Link className="size-4 text-brand" /> Active Branch Hardware Key
            </h2>
          ...
          {openSections.hardwareKey && (
            <div className="p-4 sm:p-6 border-t border-border space-y-4">
              <p className="text-xs text-muted-foreground m-0">
                Copy this unique active Branch ID credential directly into your
                physical Arduino / ESP32 RFID sketch config to seamlessly stream
                swipe updates anonymous to this branch.
              </p>
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden lg:col-span-2">
          ...
          >
            <h2 className="font-heading text-lg flex items-center gap-2 m-0">
              <Link className="size-4 text-brand" /> WhatsApp Automation Webhook
            </h2>
            <span className="text-muted-foreground font-bold">
          ...
          {openSections.webhook && (
            <div className="p-4 sm:p-6 border-t border-border space-y-3">
              <p className="text-xs text-muted-foreground m-0">
                Specify your custom API Webhook (Make/Zapier/WhatsApp tool) to
                automatically push templates upon successful RFID check-in
                scans.
              </p>
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden lg:col-span-2">
          ...
          >
            <h2 className="font-heading text-lg flex items-center gap-2 m-0">
              <Dumbbell className="size-5 text-brand" /> Custom Workout
              Templates
            </h2>
            <span className="text-muted-foreground font-bold">
          ...
          {openSections.workoutTemplates && (
            <div className="p-4 sm:p-6 border-t border-border space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <p className="text-xs text-muted-foreground m-0">
                  Customize workout templates (Day 1 - Day 6) that can be
                  assigned directly to your members.
                </p>
                <button
                  type="button"
                  onClick={addTemplate}
                  className="px-3 py-1.5 bg-secondary text-foreground hover:bg-brand/10 hover:text-brand rounded-lg text-xs font-bold inline-flex items-center gap-1.5 cursor-pointer"
                >
              ...
              <div className="space-y-4">
                {workoutTemplates.map((t, tIdx) => (
                  <div
                    key={tIdx}
                    className="p-4 bg-secondary/35 border border-border/60 rounded-2xl relative space-y-3"
                  >
                    <button
                      type="button"
                      onClick={() => removeTemplate(tIdx)}
                      className="absolute top-4 right-4 p-1.5 bg-secondary hover:bg-danger/10 hover:text-danger rounded-lg transition cursor-pointer"
                    >
                    ...
                    <div className="grid sm:grid-cols-2 gap-3 max-w-[90%]">
                      <Field label="Template Name">
                        <input
                          value={t.name}
                          onChange={(e) =>
                            updateTemplateField(tIdx, "name", e.target.value)
                          }
                          placeholder="e.g. Cardio Plan"
                          className={input}
                        />
                      </Field>
                      <Field label="Short Description">
                        <input
                          value={t.desc}
                          onChange={(e) =>
                            updateTemplateField(tIdx, "desc", e.target.value)
                          }
                      ...
                      {Array.from({ length: 6 }).map((_, dIdx) => (
                        <div key={dIdx} className="space-y-1">
                          ...
                          <input
                            value={t.days[dIdx] || ""}
                            onChange={(e) =>
                              updateTemplateDay(tIdx, dIdx, e.target.value)
                            }
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden lg:col-span-2">
          <button
            ...
            className="w-full flex items-center justify-between p-4 sm:p-6 bg-secondary/15 text-left border-0 cursor-pointer"
          >
            <h2 className="font-heading text-lg m-0 flex items-center gap-2">
              <CreditCard className="size-4 text-brand" /> Membership Plan
              Prices
            </h2>
            <span className="text-muted-foreground font-bold">
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden lg:col-span-2">
          ...
          {openSections.appearance && (
            <div className="p-4 sm:p-6 border-t border-border space-y-4">
              <p className="text-xs text-muted-foreground m-0">
                Customize theme and color preset
              </p>
                    ...
                    <div className="flex gap-2">
                      {(["dark", "light"] as ThemeMode[]).map((m) => (
                        <button
                          ...
                          onClick={() => {
                            set("theme", m);
                            gym.updateSettings({ theme: m });
                          }}
                          className={
                            "flex-1 px-4 py-3 rounded-lg text-sm border inline-flex items-center justify-center gap-2 cursor-pointer " +
                            (form.theme === m
                              ? "bg-brand text-brand-foreground border-brand font-medium"
                              : "bg-secondary border-border text-muted-foreground")
                          }
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                      {PRESETS.map((p) => (
                        <button
                          ...
                          onClick={() => {
                            set("preset", p.v);
                            gym.updateSettings({ preset: p.v });
                          }}
                          className={
                            "px-3 py-3 rounded-lg text-sm border flex flex-col sm:flex-col items-center gap-3 sm:gap-2 cursor-pointer " +
                            (form.preset === p.v
                        ...
                        >
                          <span
                            className="size-6 rounded-full ring-2 ring-border shrink-0"
        ...
        <section className="bg-card border border-border rounded-2xl overflow-hidden lg:col-span-2">
          ...
          {openSections.shifts && (
            <div className="p-4 sm:p-6 border-t border-border space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <p className="text-xs text-muted-foreground m-0">
                  Slots are auto-generated from shifts × duration
                </p>
                <button
                  onClick={addShift}
                  className="px-3 py-2 bg-secondary rounded-lg text-xs inline-flex items-center gap-1 hover:bg-brand/10 hover:text-brand font-semibold cursor-pointer"
                >
              ...
              <div className="space-y-3">
                {form.shifts.map((s, i) => (
                  <div
                    key={i}
                    className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 bg-secondary/40 rounded-xl"
                  >
                    <span className="text-xs uppercase tracking-widest text-muted-foreground w-16 shrink-0 font-bold">
                      Shift {i + 1}
                    </span>
                    ...
                    <button
                      ...
                      className="ml-auto size-9 rounded-md bg-secondary hover:bg-danger/10 hover:text-danger grid place-items-center shrink-0"
                    >
                      <Trash2 className="size-4" />
                    </button>
                  </div>
                ...
                {form.shifts.length === 0 && (
                  <p className="text-sm text-muted-foreground">
                    No shifts configured. Add one above.
                  </p>
                )}
              </div>
            </div>
        ...
        <section className="bg-card border border-border rounded-2xl p-4 sm:p-6 lg:col-span-2">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Reset all data (members, expenses, todos) to factory seed state.
          </p>
          <button
            onClick={() => {
