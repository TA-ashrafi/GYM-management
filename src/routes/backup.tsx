...
function BackupPage() {
  ...
  function exportMembers() {
    const rows = state.members.map((m) => ({
      ...
      PreferredSlot: m.preferredSlot,
      LastPunch: m.attendance[0]
        ? new Date(m.attendance[0]).toLocaleString()
        : "Never",
    ...
    state.sales.forEach((s) => {
      s.items.forEach((it) => {
        rows.push({
          ...
          Customer: s.customer ?? s.memberId ?? "Walk-in",
        });
      });
    });
  ...
  function toggleAutoBackup(v: boolean) {
    ...
    if (v) {
      ...
      toast.success(
        "Auto-backup enabled (browser localStorage already saves continuously)",
      );
    } else {
    ...
    <div className="p-8 max-w-5xl">
      <PageHeader
        ...
        actions={
          <button
            onClick={backupJson}
            className="px-5 py-2.5 bg-brand text-brand-foreground font-semibold rounded-xl text-sm inline-flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <Stat
          label="Punch Records"
          value={state.members.reduce((s, m) => s + m.attendance.length, 0)}
        />
        <Stat
          label="Sales Recorded"
          value={`${state.sales.length} · ${money(totalRevenue)}`}
        />
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        <section className="bg-card border border-border rounded-2xl p-6">
          ...
          <div className="grid gap-2">
            <ExportRow
              label="Members (with status, BMI, expiry)"
              count={state.members.length}
              onClick={exportMembers}
            />
            <ExportRow
              label="Attendance / Punch History"
              count={state.members.reduce((s, m) => s + m.attendance.length, 0)}
        ...
        <section className="bg-card border border-border rounded-2xl p-6">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Complete state — restore on any device
          </p>

          <button
            onClick={backupJson}
            className="w-full mb-2 py-2.5 bg-secondary hover:bg-brand/10 hover:text-brand rounded-xl text-sm font-semibold inline-flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="w-full py-2.5 bg-secondary hover:bg-brand/10 hover:text-brand rounded-xl text-sm font-semibold inline-flex items-center justify-center gap-2"
          >
          ...
          <label className="flex items-center justify-between mt-5 p-3 bg-secondary/40 rounded-lg cursor-pointer">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Browser already saves on every change
              </p>
            </div>
            <input
              type="checkbox"
          ...
          <div className="mt-4 p-3 bg-warn/10 border border-warn/30 rounded-lg flex gap-2 text-xs">
            ...
            <p className="text-muted-foreground">
              <strong className="text-foreground">Cloud backup tip:</strong>{" "}
              Download weekly JSON backup and store it on Google Drive /
              Dropbox.
            </p>
          </div>
        </section>
...
function ExportRow({
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center justify-between gap-3 p-3 bg-secondary/40 rounded-lg hover:bg-brand/10 hover:text-brand transition group text-left"
    >
