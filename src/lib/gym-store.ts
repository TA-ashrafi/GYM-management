...
export type SaleItem = { productId: string; qty: number; price: number; name: string };
export type Sale = {
  id: string;
  date: string;
  items: SaleItem[];
...
export function generateSlots(shifts: Shift[], durMin: number): string[] {
  ...
  for (const s of shifts) {
    ...
    // Sirf ek full slot per shift (start to end)
    out.push(
...
export const gym = {
  ...
  addMember(m: Omit<Member, "id" | "attendance">) {
    state = { ...state, members: [...state.members, { ...m, id: `mem_${Date.now()}`, attendance: [] }] };
    emit();
  },
  ...
  addExpense(e: Omit<Expense, "id">) {
    state = { ...state, expenses: [{ ...e, id: `e_${Date.now()}` }, ...state.expenses] };
    emit();
  },
  ...
  addTodo(t: Omit<Todo, "id" | "done">) {
    state = { ...state, todos: [{ ...t, id: `t_${Date.now()}`, done: false }, ...state.todos] };
    emit();
  },
  ...
  setSlotCapacity(slot: string, cap: number) {
    state = { ...state, slots: { ...state.slots, [slot]: cap } };
    emit();
  },
  ...
  setLayout(layout: DashboardWidget[]) {
    state = { ...state, settings: { ...state.settings, dashboardLayout: layout } };
    emit();
  },
  dismissNotification(id: string) {
    const ids = [...state.settings.dismissedNotifIds, id];
    state = { ...state, settings: { ...state.settings, dismissedNotifIds: ids.slice(-200) } };
    emit();
  },
  ...
  for (const m of s.members) {
    ...
    if (st === "expired") {
      out.push({
        ...
        title: `${m.name} ki membership expired`,
        desc: `${Math.abs(daysUntil(m.expiryDate))} din ho gaye — renewal pending`,
        href: "/members?filter=expired", ts: Date.now(),
      });
    } else if (st === "expiring") {
      out.push({
        id: `expg_${m.id}`, type: "expiry", tone: "warn",
        title: `${m.name} expire hone wala hai`,
        desc: `${daysUntil(m.expiryDate)} din baaki`,
      ...
      out.push({
        ...
        title: `Ghost: ${m.name}`,
        desc: `${last ? daysSince(last) : "30+"} din se punch nahi`,
