...
function load(): State {
  ...
  try {
    ...
    if (!merged.settings.dashboardLayout?.length)
      merged.settings.dashboardLayout = DEFAULT_LAYOUT;
    if (!merged.settings.theme) merged.settings.theme = "dark";
    if (!merged.settings.preset) merged.settings.preset = "lime";
    if (!merged.settings.dismissedNotifIds)
      merged.settings.dismissedNotifIds = [];
...
export const gym = {
  ...
  checkIn(id: string) {
    ...
    state = {
      ...
      members: state.members.map((m) =>
        m.id === id
          ? { ...m, attendance: [now, ...m.attendance].slice(0, 200) }
          : m,
      ),
    };
    emit();
  ...
  updateSettings(patch: Partial<Settings>) {
    ...
    const slots: Record<string, number> = {};
    newSlotList.forEach(
      (s) => (slots[s] = state.slots[s] ?? next.slotCapacity),
    );
...
export function money(
  n: number,
  c: Settings["currency"] = state.settings?.currency ?? "INR",
) {
